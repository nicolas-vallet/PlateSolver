...
import com.nzv.astro.platesolver.service.impl.MockedCatalogServiceImpl;
...
public class StarDetector {
	...
	private final static CatalogService catalogService = new MockedCatalogServiceImpl();
	...
	public static void main(String[] args) throws BeansException, InterruptedException, ExecutionException {
		...
		System.out.println("We found " + components.size() + " component" + (components.size() > 1 ? "s" : "")
				+ " in channel " + WORKING_CHANNEL + ", in " + (tEnd - tStart) + " ms.");
		idx = 0;
		...
		List<Triangle> opmSortedTriangles = astroService
				.buildTrianglesFromComponents(components.subList(0, Math.min(components.size(), MAX_COMPONENTS)));
		tEnd = System.currentTimeMillis();
		System.out.println(
				"We made " + opmSortedTriangles.size() + " triangle" + (opmSortedTriangles.size() > 1 ? "s" : "")
						+ " based on discovered components, in " + (tEnd - tStart) + " ms.");
		for (Triangle triangle : opmSortedTriangles.subList(0, Math.min(opmSortedTriangles.size(), 8))) {
			System.out.println(triangle);
		}

		// On cherche les étoiles candidates dans le catalogue...

		Optional<DeepSkyObject> optTarget = catalogService.findObjectByName(TARGET_NAME);
		if (optTarget.isPresent()) {
			DeepSkyObject target = optTarget.get();
			...
			System.out.println(sb.toString());
			tStart = System.currentTimeMillis();
			List<CatalogObject> stars = catalogService.findBrightestStarsInArea(upperWesternAreaCorner,
					lowerEasternAreaCorner);
			tEnd = System.currentTimeMillis();
			sb = new StringBuilder();
			idx = 0;
			...
			System.out.println("We found " + stars.size() + " catalog object" + (stars.size() > 1 ? "s" : "") + " in "
					+ (tEnd - tStart) + " ms.");
			System.out.println(sb.toString());

			tStart = System.currentTimeMillis();
			List<Triangle> catalogTriangles = astroService
					.buildTrianglesFromCatalogObjects(stars.subList(0, Math.min(stars.size(), MAX_COMPONENTS)));
			tEnd = System.currentTimeMillis();
			System.out.println(
					"We made " + catalogTriangles.size() + " triangle" + (catalogTriangles.size() > 1 ? "s" : "")
			...
			for (Triangle catalogTriangle : catalogTriangles) {
				if ((int) catalogTriangle.getLongestShortestSidesRatio() == (int) opmSortedTriangles.get(0)
						.getLongestShortestSidesRatio()) {
